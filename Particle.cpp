...
Particle::Particle(glm::vec2 StartPos, float I_size)
	...
	, m_k(1.0f), m_AnimationSpeed(0.1f), m_CurrentVelocity({ 0.0f,0.0f }),m_Gravity{0,100.0f}
...
void SweepAndPrune(std::vector<Particle> &Particles)
{
	for (size_t i = 0; i < Particles.size()-1; ++i)
	{
		for (size_t j = i+1; j < Particles.size(); j++)
		{
			...
			if (dist < Particles[i].m_Size )
			{
										...
										Particles[i].m_CurrentPosition += 0.01f * delta * n;
										Particles[j].m_CurrentPosition -= 0.01f * delta * n;
			}
		}
	...
	//					{
	//						if (Current->GetCastedShadowX().y <= Checked->GetCastedShadowX().x ||
	//							Current->GetCastedShadowY().y <= Checked->GetCastedShadowY().x ||
	//							Current->GetCastedShadowX().x >= Checked->GetCastedShadowX().y ||
	//							Current->GetCastedShadowY().x >= Checked->GetCastedShadowY().y ||
	//							(dx==0 &&dy==0)
	//							)continue;
	...
	//					
	//							const glm::vec2 n = Collision_axis / dist;
	//							const float delta = Current->m_Size * 2 - dist;
	//							Current->m_CurrentPosition += 0.05f * delta * n;
	//							Checked->m_CurrentPosition -= 0.05f * delta * n;
